...
def test_validation(model, testloader):
    test_loss = 0
    accuracy = 0
    model.eval()
    model.to(device)
...
test_validation(model, testloader)
...
ps, classes = predict('flowers/test/10/image_07090.jpg', model)
...
# First plot
picture = plt.subplot(2, 1, 1)

image = process_image('flowers/test/15/image_06351.jpg')

ps, classes = predict('flowers/test/15/image_06351.jpg', model)

flower_name = cat_to_name['15']
imshow(image, picture, title=flower_name)
...
# second plot
plt.subplot(2, 1, 2)
sb.barplot(x=ps, y=Flower_names, color=sb.color_palette()[0])
